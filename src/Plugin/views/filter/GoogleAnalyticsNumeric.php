...
class GoogleAnalyticsNumeric extends GoogleAnalyticsBase {
  ...
  public function adminSummary() {
    ...
    elseif (\in_array($this->operator, $this->operatorValues(1), TRUE)) {
  ...
  public function valueForm(&$form, FormStateInterface $form_state) {
    ...
    if (!empty($values['exposed'])) {
      ...
      if (
        ...
        // Exposed and locked.
        $which = \in_array($this->operator, $this->operatorValues(2), TRUE)
          ? 'minmax'
          : 'value';
      }
      else {
    ...
    if ($which === 'all') {
      ...
      if (
        !empty($values['exposed'])
        && !isset($values['input'][$identifier]['value'])
      ) {
    ...
    if ($which === 'all' || $which === 'minmax') {
      ...
      if (
        !empty($values['exposed'])
        && !isset($values['input'][$identifier]['min'])
      ) {
      ...
      if (
        !empty($values['exposed'])
        && !isset($values['input'][$identifier]['max'])
      ) {
