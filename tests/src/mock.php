...
function mock_drupal_services($item, $config_map = []) {
  foreach ([
    ...
    ['\\Drupal\\Core\\Entity\\EntityTypeManager', 'entity_type.manager', function ($item, &$themock) {
      ...
      // }],
      // ['\\Drupal\\Core\\Entity\\EntityManager', 'entity.manager', function($item, &$themock) {
      $file = $item->getMockBuilder('Drupal\file\Entity\File')
        ->disableOriginalConstructor()
        ->getMock();
      $file->expects($item->any())
        ->method('getFileUri')
        ->willReturn('/tmp.jsonfileuri.json');
      $storage = $item->getMockBuilder('Drupal\file\FileStorage')
        ->disableOriginalConstructor()
        ->getMock();
      $storage->expects($item->any())
        ->method('load')
        ->will($item->returnValueMap([['11', $file]]));
      $themock->expects($item->any())
        ->method('getStorage')
        ->with('file')
        ->willReturn($storage);
